...
export function DishesSection({ categories, items }: DishesSectionProps) {
  ...
  const effectiveNewItemCategoryId =
    categories.some((category) => category.id === newItem.categoryId)
      ? newItem.categoryId
      : (categories[0]?.id ?? "");
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => item.category_id === effectiveSelectedCategoryId);
  }, [effectiveSelectedCategoryId, items]);
  ...
  async function handleCreateItem() {
    ...
    if (!effectiveNewItemCategoryId || !name || Number.isNaN(price)) {
      return;
    }

    await runWithErrorHandling(async () => {
      await createMenuItem({
        categoryId: effectiveNewItemCategoryId,
        name,
        price,
      });
    ...
    <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-2">
            ...
            return (
              <div
                ...
                className={`flex min-h-10 items-center justify-between gap-2 rounded-xl border px-2 py-2 ${
                  isSelected ? "border-indigo-300 bg-indigo-50" : "border-gray-200 bg-gray-50"
                }`}
              >
                <button
                  type="button"
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-4 text-gray-900 shadow-sm sm:p-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <select
            value={effectiveNewItemCategoryId}
